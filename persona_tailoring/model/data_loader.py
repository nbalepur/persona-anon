...
class DataLoader:
    ...
    def __init__(self, dataset_split, dataset_name=config.params['hf_dataset_name']):
    ...
    def load_few_shot_data(self, persona: Persona):
        ...
        return datasets.Dataset.from_dict(train_data)
    
    def load_sft_data(self, training_type: TrainingType):
        """Load data for supervised fine-tuning (SFT)."""
        """Output: Training Dataset, Validation Dataset with columns `prompt`, `persona`, `response`"""
    ...
    def load_dpo_data(self, training_type: TrainingType):
        ...
        """Output: Training Dataset, Validation Dataset with columns `prompt`, `persona`, `chosen`, and `rejected`"""
    ...
    def load_test_data(self, persona: Persona):
        ...
        """Output: Testing Dataset with columns `prompt`, `persona`"""

        system_persona = self.system_persona_map.get(self.dataset_split, "")
        ...
        test_data = {'prompt': ds_test['prompt'], 'persona': test_personas}
