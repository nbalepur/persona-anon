...
def setup_model_and_tokenizer(persona, cache_dir):
    ...
    dpo_model_name = f"{config.params['dpo_final_output_dir']}_{persona.training_type.value}"
    tokenizer_name = config.params['sft_tokenizer_name']

    # Check that models exist
    for file in [tokenizer_name, dpo_model_name]:
        model_path = Path(file)
        if not model_path.exists() or not model_path.is_dir():
            raise FileNotFoundError(f"Model directory '{file}' does not exist. Please ensure the DPO model is trained or provide the correct arguments.")
...
def create_output_directory():
    ...
    results_dir = Path(config.params['dpo_results_dir']).parent
...
def main(args):
    ...
    test_prompt_template = fetch_testing_template(training_type=persona.training_type, inference_type=persona.inference_type, dataset_split=config.params['dataset_name'])
    ...
    # Generate text
    output_file = Path(f"{config.params['dpo_results_dir']}_{persona}.jsonl")
    stop_token = '\nPrompt:'
