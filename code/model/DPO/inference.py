...
def main():
    ...
    dpo_model_name = f"{config.params['dpo_final_output_dir']}_{use_persona}_{response_type}"
...
{response_prefix[idx]}""" for idx in range(len(ex['prompt']))]}
            ...
            return {'prompt': [f"""Prompt: {ex['prompt'][idx]} {ex['persona'][idx].replace('The user is', 'I am')}
{response_prefix[idx]}""" for idx in range(len(ex['prompt']))]}
            
    if config.params['dataset_name'] == 'Mnemonic':    
        INFERENCE_TYPE = [] if use_persona else ['retr-rejected', 'retr']
    else:
        INFERENCE_TYPE = [] if use_persona else ['retr-rejected', 'retr', 'oracle', 'oracle-rejected']
    ...
    for inference_type in INFERENCE_TYPE:
        ...
        path = '/'.join(config.params['dpo_results_dir'].split('/')[:-1])
        if not os.path.exists(path):
            os.makedirs(path)
    
        f = f"{config.params['dpo_results_dir']}_{use_persona}_{inference_type}_{response_type}.jsonl"
        ...
        for idx in tqdm.tqdm(range(len(outputs), len(inf_prompts))):
            ...
            out = dpo_model.generate(input_ids=input_ids, attention_mask=attention_mask, min_new_tokens=10, max_new_tokens=512, do_sample=False, stopping_criteria=stopping_criteria).to('cpu').detach()
